...
import Html exposing (Html, button, div, input, select, text,label)
import Html.Attributes exposing (href, type_,checked)
...
import Model.PostsConfig exposing (applyChanges, Change(..))
    ...
    let
        ( newState, cmd ) =
            case ( model.state, msg ) of
                ...
                ( Model.LoadedPosts state, Model.ConfigChanged change ) ->
                    let
                        updatedConfig = Model.PostsConfig.applyChanges change state.config --se aplica modificarile pe tabel in functie de configurarea alegerilor
                    in        
